...
class TestGetTransactions:
    async def test_returns_empty_list_when_no_transactions(self, client, db_session):
        ...
        response = await client.get("/endpoint/v1/transactions")
...
class TestPostTransaction:
    async def test_create_deposit_success(self, client, db_session):
        ...
        assert balance.amount == Decimal("200.0")
    ...
    async def test_create_withdraw_success(self, client, db_session):
        ...
        balance = balance.scalar_one()
        assert balance.amount == Decimal("350.0")

    async def test_invalid_currency(self, client, db_session):
        await create_user_with_balances(db_session, email="tester@test.com", status=UserStatusEnum.ACTIVE)
        response = await client.post(
            "/endpoint/v1/transactions",
            json={"currency": "INVALID", "amount": "100"},
        )
    ...
    async def test_insufficient_balance(self, client, db_session):
        user = await create_user_with_balances(db_session, email="tester@test.com", status=UserStatusEnum.ACTIVE)
        response = await client.post(
            "/endpoint/v1/transactions",
    ...
    async def test_zero_amount(self, client, db_session):
        user = await create_user_with_balances(db_session, email="tester@test.com", status=UserStatusEnum.ACTIVE)
        response = await client.post(
            "/endpoint/v1/transactions",
...
class TestPatchRollbackTransaction:
    async def test_rollback_deposit(self, client, db_session):
        ...
        response = await client.patch(
            f"/endpoint/v1/transactions/{tx_id}",
        )
        assert response.status_code == 200
        ...
        balance = balance.scalar_one()
        assert balance.amount == Decimal("0.0")
    ...
    async def test_rollback_withdraw(self, client, db_session):
        ...
        balance = balance.scalar_one()
        assert balance.amount == Decimal("500.0")

    async def test_already_rollbacked(self, client, db_session):
        user = await create_user_with_balances(db_session, email="tester@test.com", status=UserStatusEnum.ACTIVE)
        ...
        await client.patch(f"/endpoint/v1/transactions/{tx_id}")
    ...
    async def test_transaction_not_found(self, client, db_session):
        await create_user_with_balances(db_session, email="tester@test.com", status=UserStatusEnum.ACTIVE)
        fake_tx_id = "01ARZ3NDEKTSV4RRFFQ69G5FAV"
        response = await client.patch(
            f"/endpoint/v1/transactions/{fake_tx_id}",
        )
        assert response.status_code == 404

    async def test_transaction_does_not_belong_to_user(self, client, db_session):
        ...
        response = await client.patch(
            f"/endpoint/v1/transactions/{tx.id}",
        )
        assert response.status_code == 400
